...
<body style="background-image: url(statics/img/heroList.png);background-size: cover;">
	...
	<div align="center">
		<div>
			<form id="searchForm" method="post" action="hero/queryLike" style="color:white">
		...
		<div >
			<table style="background-color:white; color: black; background-color: #ffffffaa; width: 700px; text-align: center">
				<tr>
					<th>编号</th>
					<th>角色名</th>
					<th>职业群</th>
				...
				<c:forEach items="${heroList }" var="hero">
					<tr>
						<td>${hero.id }</td>
						<td><a href="hero/detail/${hero.id }">${hero.name }</a></td>
						<td>${hero.race }</td>
						<td>${hero.type }</td>
					</tr>
				</c:forEach>
